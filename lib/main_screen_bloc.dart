...
class MainScreenBloc with ChangeNotifier {
  ...
  Future<void> fetch() async {
    await _prefs.then((value) {
      setChildName(value.getString('childName') ?? "");
    });

    DateTime childBirthday = DateTime.now();
    await _prefs.then((value) {
      childBirthday = DateTime.fromMillisecondsSinceEpoch(value.getInt('childBirthday') ?? 0);
      setChildBirthday(childBirthday);
    }).whenComplete(() {
      fetchVaccinList(childBirthday);
    });
  }

  Future<void> fetchVaccinList(DateTime birthday) async {
    final dir = await getApplicationDocumentsDirectory();
    isar = await Isar.open(
      [VaccinSchema],  // 사용할 schema 다 명시
      directory: dir.path,
    );

    final vaccinList = isar.vaccins;
    if (vaccinList.countSync() == 0) {
      ...
      await isar.writeTxn(() async {
        await isar.vaccins.putAll(vaccinListNotifier.value);
      });
    } else {
      vaccinListNotifier.value = await isar.vaccins.where().findAll();
    }
  }
  ...
  Future<void> setVaccinMemo(int index, String memo) async {
    vaccinListNotifier.value[index] =
        vaccinListNotifier.value[index].copyWith(memo: memo);
    vaccinListNotifier.notifyListeners();

    await isar.writeTxn(() async {
      await isar.vaccins.put(vaccinListNotifier.value[index]);
    });
  }
